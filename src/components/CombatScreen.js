...
export const CombatScreen = () => {
  ...
  const addFloat = (val, type) => {
    ...
    // Heal floats on the right, damage on the left, randomize position
    const x = type === 'heal' ? 650 + Math.random() * 60 : 380 + Math.random() * 60;
    const y = 150 + Math.random() * 150;
    const offsetX = (Math.random() - 0.5) * 40; // Horizontal spread
    setFloats(p => [...p, { id, x, y, val, type, offsetX, startTime: Date.now() }]);
    setTimeout(() => setFloats(p => p.filter(f => f.id !== id)), 1500);
  };
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-around px-6 relative">
          ...
          {floats.map(f => {
            ...
            const offsetY = -120 * progress;
            const opacity = Math.max(0, 1 - progress);
            const fontSize = f.type === 'critical' ? '28px' : f.type === 'heal' ? '24px' : '22px';
            const fontWeight = f.type === 'critical' ? '900' : '800';
            
            return (
              <div key={f.id}
                className={`absolute font-black pointer-events-none drop-shadow-lg
                  ${f.type==='heal'?'text-green-400':f.type==='critical'?'text-amber-300':'text-red-400'}`}
                style={{
                  left: f.x + f.offsetX,
                  top: f.y + offsetY,
                  opacity,
                  ...
                  transform: `translate(-50%, -50%) scale(${1 + (f.type === 'critical' ? 0.3 : 0.1) * progress})`
                }}>
                {f.type === 'critical' ? '⚡ ' : ''}{f.type==='heal'?'+':'-'}{f.val}{f.type === 'heal' ? ' ✨' : ''}
              </div>
            );
          })}
        </div>
